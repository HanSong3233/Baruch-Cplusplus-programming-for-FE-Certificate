...
public:
	...
	//access functions
	double X() const; //access X£¬we rename it from Get_X to X, we need to make it constant in this problem.
	double Y() const; //access Y, we rename it from Get_Y to Y, we need to make it constant in this exercise.
    string ToString() const;//returns a string description of the point, we need to make it constant in this exercise.
	double Distance() const;//Calculate the distance of point to origin, we rename it from DistanceOrgin to Distance
	                        //we need to make it constant in this exercise. 
