...
#include "Supplementfunction.hpp"//include the header file of supplement function which contain some self-defined useful function.  
...
{
	if (IsCall())  // if it is a call option.
		return ((N(m_d1)) * m_S*exp((m_b-m_r)*m_T)) - (m_K * exp(-m_r * m_T) * N(m_d2));
	else    //if it is a put option. 
		return (N(-m_d2) * m_K * exp(-m_r * m_T)) - (m_S*exp((m_b - m_r)*m_T) * N(-m_d1));
}

double EuropeanOption::Price(const double &S) const
{
	double d1 = (log(S / m_K) + ((m_b + ((m_sig * m_sig) / 2.0)) * m_T)) / (m_sig * sqrt(m_T));  //updated the d1 with S explicitly given.
	double d2 = (log(S / m_K) + ((m_b - ((m_sig * m_sig) / 2.0)) * m_T)) / (m_sig * sqrt(m_T));   //updated the d2 with S explicitly given.
...
//return the delta of function with S explicitly given, according to the exact solution. 
...
//return the gamma of the European Option calculated by the exact method
double EuropeanOption::Gamma() const
{
	return n(m_d1)*exp((m_b - m_r)*m_T)/(m_S*m_sig*sqrt(m_T));
}

//return the gamma of the European Option with the S explicitly given, calculated by the exact method
...
//calculate the delta with divided difference method.
...
//calculate the Gamma with function with devided difference method. 
