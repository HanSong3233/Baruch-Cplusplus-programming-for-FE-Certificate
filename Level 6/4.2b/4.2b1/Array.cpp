...
{
	m_size = default_size;  // Defaulted to 10 elements
	m_data = new T[m_size]; //the default set is a array of ten points, all of them is (0,0)
}
...
//Accessing member-function for obtaining default size of the array, return the defalut_size
template <class T>
int Array<T>::GetDefaultSize() { return default_size; }
...
//setter function for setting the default size. 
template <class T>
void Array<T>::DefaultSize(int a)
{
	default_size = a;
}
